...
const Residents = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fetchResidents();
      setShowModal(false);
      setFormData({ full_name: '', resident_status: 'tetap', phone_number: '', marital_status: 'belum', ktp_photo: null });
    } catch (error) {
      console.error(error);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: '800' }}>Manajemen Penghuni</h1>
          <p style={{ color: 'var(--text-secondary)' }}>Kelola data seluruh warga perumahan.</p>
        </div>
          ...
          <tbody>
            {residents.map(r => (
              <tr key={r.id}>
                ...
                <td>
                  <span className={`badge ${r.resident_status === 'tetap' ? 'badge-success' : 'badge-warning'}`}>
                ...
                <td style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  <button className="btn btn-outline" style={{ padding: '0.5rem', color: 'var(--danger)' }} onClick={() => handleDelete(r.id)}><Trash2 size={16} /></button>
                </td>
              </tr>
            ))}
          </tbody>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  ...
                  <select value={formData.resident_status} onChange={e => setFormData({...formData, resident_status: e.target.value})}>
              ...
              <div>
                ...
                <input type="text" required value={formData.phone_number} onChange={e => setFormData({...formData, phone_number: e.target.value})} />
              </div>
              <div>
                <label>Foto KTP</label>
                <input type="file" onChange={e => setFormData({...formData, ktp_photo: e.target.files[0]})} />
              </div>
